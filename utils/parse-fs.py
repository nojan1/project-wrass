        ...
        for data in sectorsData:
            for i in range(int(BPB_BytsPerSec / 32)):
                ...
                (
                    ...
                    DIR_FstClusLO,
                    DIR_FileSize,
                ) = unpack("11sBBBHHHHHHHI", entryData)
                ...
                print(
                    f"{"DIR" if isDirectory else extension}\t{baseName}\t{DIR_Attr:02X}\t{DIR_FileSize}\t{cluster}\t{dosdate(DIR_WrtTime, DIR_WrtDate)}\t{dosdate(DIR_CrtTime, DIR_CrtDate)}"
                )
...
def getSectorDataForCluster(clusterNum):
    ...
    print(
        f"Calculated sector offset for cluster {clusterNum} is {firstSectorOfCluster:08X}"
    )
...
def parseHeaders():
    ...
    print(
        f"cluster_begin_lba={cluster_begin_lba:08X} ({cluster_begin_lba}) - byte offset {cluster_begin_lba * 512}"
    )
